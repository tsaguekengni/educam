...
- **2026-09-13 (11)** — ✅ **MESSAGES HORS LIGNE — dernier élément du périmètre retenu.** Trois problèmes, pas un seul. (a) **L'expéditeur venait d'un appel SERVEUR** (`supabase.auth.getUser()`) : hors ligne il ne renvoyait rien et le message aurait été enregistré **sans expéditeur**. Remplacé par le profil déjà en mémoire (`teachers.id` **est** l'identifiant d'authentification). (b) **Les listes de destinataires n'étaient pas en cache** : hors ligne, aucun élève ni collègue à qui écrire — le composeur était inutilisable. Mises en cache, ainsi que la correspondance **élève → compte parent**, résolue à l'ouverture du composeur pendant qu'on a le réseau, puisque l'envoi en a besoin et ne peut pas la chercher sans. (c) **La notification WhatsApp doit suivre la création du message** : l'identifiant `messages.id` étant un uuid, il est **généré sur l'appareil**, le message et la notification sont mis en file **l'un derrière l'autre**, et comme la file se vide du plus ancien au plus récent, la ligne existe avant qu'on demande de notifier. Nouvelles opérations de file : `update` et `invoke` (fonction Edge). ⚠️ La notification porte `bestEffort: true` — **un échec la retire de la file au lieu de bloquer ce qui suit** : une relance WhatsApp ne doit jamais retenir les notes d'une enseignante. **Marquer un message lu** passe aussi par la file (sinon il restait lu uniquement sur cette machine, et la liste « parents à relancer » du directeur continuait de le signaler). **Défaut rattrapé avant envoi :** `WHATSAPP_ENABLED` était utilisé sans être importé — la vérification de syntaxe ne voit pas ce genre d'erreur, seul un contrôle « chaque nom est-il dans la portée ? » l'attrape. À ajouter au contrôle d'après-lot, avec la recherche de chaînes dans le paquet déployé.
- **2026-09-13 (10)** — 🔴 **L'accueil s'est figé après le lot « cache d'abord » — cause trouvée : `indexedDB.open` peut ne JAMAIS répondre.** Symptôme : les autres écrans fonctionnaient, seul l'**Accueil** basculait après quelques secondes sur la page d'erreur du navigateur ; le problème a disparu de lui-même après ~15 minutes. **Cause :** `openDB()` ne gérait pas `onblocked`. Quand **une autre fenêtre ou un autre onglet** garde la base ouverte sur une version antérieure, la montée de version est **bloquée** et `indexedDB.open` ne déclenche **ni succès ni erreur** — la promesse ne se règle jamais. Or ce lot venait d'ajouter **trois lectures de base sur le chemin de rendu de l'Accueil** (statistiques de classe, plan de la séance, messagerie) : elles attendaient donc indéfiniment. Les autres écrans n'attendent rien de la base, d'où l'écran unique touché. Le déclencheur exact : un onglet Chrome ouvert en parallèle sur l'ancien code (pour lire la console) retenait la base en **version 1** pendant que l'application installée demandait la **version 2**. **Correctif :** connexion unique et partagée ; `onblocked` → échec rapide au lieu d'attente infinie ; `onversionchange` → on libère la base pour que l'autre fenêtre puisse migrer ; `onclose` → réouverture propre ; plus un plafond de 4 s en dernier recours. Les appelants traitent un échec comme « pas de copie locale », ce qui est récupérable — **l'attente infinie ne l'était pas.** ⚠️ **À retenir : deux fenêtres sur des versions différentes du code suffisent à bloquer la base locale.** Le danger augmente à chaque changement de `DB_VERSION`.
- **2026-09-13 (9)** — ✅ **Horodatage réel confirmé en base, et premier lot de « lectures cache d'abord ».** Preuve du correctif : trois notes saisies à une minute d'intervalle sont arrivées à **15:21:00 / 15:22:36 / 15:23:32** — contre **0,16 s d'écart** au test précédent. Confirmation indépendante : les nouvelles dates ont trois décimales (millisecondes, donc produites par le navigateur), les anciennes six (microsecondes, donc par Postgres). Les heures viennent bien de l'appareil de l'enseignante. **Lot suivant construit :** `cachedQueryMeta()` remplace la logique réseau-d'abord — hors ligne, la copie locale est rendue **immédiatement, sans requête** ; en ligne, requête plafonnée à 5 s avec repli sur la copie locale ; dans les deux cas `cachedAt` alimente un repère « au 11 sept. · 14 h 20 » (`freshnessLabel`). Rétrocompatible : les entrées de cache écrites avant ce changement sont lues telles quelles. **Trois lectures de l'accueil enseignant y passent** : `classStats` (les deux tuiles qui mentaient), le plan de la prochaine séance, et la messagerie. Les tuiles ne disent plus jamais « chargement… » une fois l'essai terminé : soit les chiffres avec leur date, soit « indisponible hors ligne ». Restent en direct les tuiles **superadmin** (en ligne par nature, hors périmètre).
- **2026-09-13 (8)** — 🟠 **Deuxième chemin « marquer enseignée » trouvé — et c'est le principal.** Après déploiement, analyse des fichiers JavaScript **réellement servis** en cherchant l'ancienne phrase de refus : elle y était encore. Origine : `markHeroTaught` (`dashboard.js`), le bouton posé sur le tableau de bord juste après le cours — son propre commentaire le décrit comme « le geste qui suit immédiatement le cours », donc **plus fréquent** que celui du lecteur de leçon. `toggleTaught` avait été corrigé, celui-ci non : il écrivait encore en direct et refusait hors ligne. Corrigé à l'identique (file + `taught_at` explicite). **Leçon de méthode : chercher dans le paquet déployé une phrase censée avoir disparu est un contrôle bon marché et efficace** — à refaire après chaque lot. Audit complet des écritures restantes : ne restent en direct que l'**administration** (écoles, comptes, édition de leçons — en ligne par nature) et, côté enseignant, **messages** (lot suivant, déjà prévu), **message lu** et **commentaire de leçon** (hors périmètre retenu — à arbitrer).
- **2026-09-13 (7)** — ✅ **FILE D'ATTENTE VALIDÉE SUR APPAREIL RÉEL.** Saisie hors ligne, leçon marquée enseignée hors ligne, remontée automatique au retour du réseau : tout est passé. **Mais la base a révélé une erreur que l'application ne montrait pas** : trois notes saisies à des moments différents sont arrivées à **0,3 seconde d'intervalle** (15:02:49.356 / .516 / .679) — elles portaient l'heure de la **synchronisation**, pas celle de la saisie. Cause : `created_at` avait bien été ajouté explicitement à `lessons_taught` et `activity_log`… mais **oublié sur `daily_results`**, la table la plus importante des trois. Le §4 décrivait précisément ce piège. Corrigé. *(À retenir : `result_date` restait juste — les notes étaient rattachées au bon jour d'école ; c'est l'heure de SAISIE, celle que lit la détection anti-triche, qui était fausse. Une vérification en base a trouvé ce qu'un test d'interface réussi ne pouvait pas voir.)*
- **2026-09-13 (7b)** — **Liste « Leçon du jour » rendue lisible** (retour Maxime). Elle n'affichait que « U1 · titre » : ni matière, ni centre d'intérêt, donc impossible de reconnaître la leçon cherchée. Désormais : **groupée par matière** (`<optgroup>`), et le centre d'intérêt rejoint l'intitulé — « Français et Littérature » → « U1 · La nature · Les adjectifs indéfinis ». `SUBJECT_NAMES` ajouté à `lib/theme.js` (et non importé de `dashboard.js`, 280 Ko). Clé de cache passée en `v2` car les colonnes lues ont changé.

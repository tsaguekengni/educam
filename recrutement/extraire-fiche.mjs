// Extrait la structure de fiche.html en JSON, pour que le Word soit engendré
...
await p.goto('file://' + process.cwd() + '/fiche.html', { waitUntil: 'domcontentloaded' });
...
await b.close();
fs.writeFileSync('_texte-fiche.json', JSON.stringify(blocs, null, 1));
const n = (t) => blocs.filter((b) => b.t === t).length;
console.log(`✓ _texte-fiche.json — ${blocs.length} blocs ` +

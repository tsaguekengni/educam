...
await p.goto('file://' + process.cwd() + '/fiche.html', { waitUntil: 'networkidle' });
await p.waitForTimeout(1200);
await p.emulateMedia({ media: 'print' });
await p.pdf({
  path: 'EduCam_Fiche_Poste_Referent.pdf', format: 'A4', printBackground: true,
  ...
  footerTemplate: `
    <div style="width:100%;font-family:Arial,sans-serif;font-size:7pt;color:#5A6470;
      ...
      <span><b style="color:#0F4C35">EduCam</b> · Fiche de poste — Mansa Musa Academy · Août 2026</span>
      <span>Page <span class="pageNumber"></span> / <span class="totalPages"></span></span>
    </div>`,
});
const n = await p.evaluate(() => document.querySelectorAll('.art-hd').length);
await b.close();
console.log(`✓ EduCam_Fiche_Poste_Referent.pdf — ${n} sections`);
